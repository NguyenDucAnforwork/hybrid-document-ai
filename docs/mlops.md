...
`doctype`, stage+lineage trong cùng registry). Hiện có **3 loại**: **receipt** (KIE sklearn), **bank_statement** (header KIE + **table parsing**
`docai/statement.py`), **payment_order/ủy nhiệm chi** (anchor KV `docai/kv.py`). Đo trên **HARD**
test (`docs/logs/multidoc_*.md`): 3-way routing 1.0, statement row-F1 0.99 **nhưng amount-acc
0.35 / desc-acc 0.55** (rule-based table parsing KHÔNG generalize qua nhiều layout bank → cần
learned table model / VLM cho statement — finding trung thực, xem README). Thêm loại mới (eKYC/ID,
FUNSD form) = 1 entry registry + optional extractor; serving/MLOps/VLM dùng lại. **Anti-fraud**
dùng chung quality+confidence+drift.
